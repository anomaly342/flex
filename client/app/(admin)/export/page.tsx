...
export default function ExportDataPage() {
  ...
  const chartOptions: ChartOptions<"line"> = {
    ...
    plugins: {
      legend: { position: "top" },
      title: { display: true, text: `Data Preview (${duration})` },
    ...
    elements: {
      point: {
        radius: (ctx) => {
          ...
          let originalIndex = dataIndex;
          if (selectedRange.start !== null) {
            originalIndex += selectedRange.start;
          }
          ...
          const isStartPoint =
            ...
            originalIndex === selectedRange.start;
          const isEndPoint =
            selectedRange.end !== null && originalIndex === selectedRange.end;

          return isStartPoint || isEndPoint ? 8 : 4;
        },
        backgroundColor: (ctx) => {
          const dataIndex = ctx.dataIndex;
          let originalIndex = dataIndex;
          ...
          return isStartPoint || isEndPoint ? "red" : "rgba(0,0,0,0.1)";
        },
      },
    },
  };
  ...
  const handleExport = () => {
    const exportData = filteredData.map((d) => {
      ...
      filters.forEach((f) => (row[f] = d[f]));
      return row;
    });

    let content = "";
    if (fileType === ".csv") {
      const header = ["Date", ...filters.map((f) => f)];
      const rows = exportData.map((d) => [d.date, ...filters.map((f) => d[f])]);
